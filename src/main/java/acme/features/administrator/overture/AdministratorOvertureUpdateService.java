...
public class AdministratorOvertureUpdateService implements AbstractUpdateService<Administrator, Overture> {
	...
	public void validate(final Request<Overture> request, final Overture entity, final Errors errors) {
		...
		//Money validation
		boolean money = entity.getMinimumMoney().getAmount() < entity.getMaximumMoney().getAmount() && entity.getMinimumMoney().getAmount() >= 0 && entity.getMaximumMoney().getAmount() >= 0;
		...
		if (!errors.hasErrors("minimumMoney") && !errors.hasErrors("maximumMoney")) {
			errors.state(request, !currency, "maximumMoney", "acme.validation.moneyCurrency");
		}
